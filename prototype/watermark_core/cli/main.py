...
from watermark_core.detect import decode_watermark, read_image
...
def cmd_detect(args):
    src = read_image(args.input)
    expected = build_payload(args.unix_time, args.t0, args.device_id)
    t = compose_template(len(src[0]), len(src), to_bitstream(expected))
    result = decode_watermark(
        src,
        t,
        expected_device_id=args.device_id,
        expected_time_slot=expected.time_slot,
        time_slot_window=args.time_slot_window,
    )
...
def build_parser():
    ...
    p4.add_argument("--time-slot-window", type=int, default=8, help="允许与期望 time_slot 的偏差范围（单位：slot）")
