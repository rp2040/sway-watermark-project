...
- `T0` 默认值：`2026-01-01 00:00:00 UTC`
...
```bash
PYTHONPATH=prototype python -m watermark_core.cli.main payload --unix-time 1767225600
```
...
```bash
PYTHONPATH=prototype python -m watermark_core.cli.main embed --input prototype/samples/input.ppm --output prototype/samples/wm.ppm --unix-time 1767225600 --alpha 0.35
```

## 11.5 解码图像（支持 .ppm / .png）

```bash
PYTHONPATH=prototype python -m watermark_core.cli.main detect --input prototype/samples/photo.ppm --unix-time 1774218140 --time-slot-window 16
```

说明：`--time-slot-window` 用于约束结果与期望 time_slot 的最大偏差（单位：slot，1 slot = 5 分钟），有助于降低真实截图中的误选概率。
