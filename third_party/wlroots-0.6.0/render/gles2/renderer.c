...
static bool gles2_render_texture_with_matrix(struct wlr_renderer *wlr_renderer,
	...
	glUniform2f(shader->wm_viewport, (float)renderer->viewport_width,
...
struct wlr_renderer *wlr_gles2_renderer_create(struct wlr_egl *egl) {
	...
	renderer->shaders.tex_rgba.wm_viewport = glGetUniformLocation(prog, "wm_viewport");
	...
	renderer->shaders.tex_rgbx.wm_viewport = glGetUniformLocation(prog, "wm_viewport");
	...
	if (renderer->exts.egl_image_external_oes) {
		...
		renderer->shaders.tex_ext.wm_viewport = glGetUniformLocation(prog, "wm_viewport");
